...
<body>
    ...
    @php
        ...
        $merchants = App\Models\Merchant::query()
            ...
            ->with(['branch'])
            ->withCount([
                'parcel' => function ($query) use ($start_date, $end_date) {
                    $query->whereBetween('status', [0, 10])->whereBetween('created_at', [$start_date, $end_date]);
                },
            ])
            ->withCount([
                'parcel as picked_up' => function ($query) use ($start_date, $end_date) {
                    $query
                        ->whereBetween('parcels.status', [11, 36])
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <tfoot>
                            <tr class="table-secondary">
                                ...
                                <td>{{ $merchants->sum('parcel_count') }}</td>
                                <td>{{ $merchants->sum('picked_up') }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
