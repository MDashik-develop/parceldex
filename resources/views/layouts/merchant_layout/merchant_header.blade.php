@php
    ...
    $x = App\Models\Parcel::where('merchant_id', $merchant_id)
        ->where('status', '>=', 25)
        ->whereIn('delivery_type', [1, 2, 4])
        ->whereIn('payment_type', [2, 4, 6])
        ->sum('customer_collect_amount');

    $y = App\Models\Parcel::where('merchant_id', $merchant_id)
        ->where('status', '>=', 25)
        ->whereIn('delivery_type', [1, 2, 4])
        ->whereIn('payment_type', [2, 4, 6])
    ...
    $total_charge_amount_query = App\Models\Parcel::where('merchant_id', $merchant_id)
        ->where('status', '>=', 25)
        ->whereIn('delivery_type', [1, 2, 4])
        ->whereIn('payment_type', [2, 4, 6]);
    ...
    $return_charge_amount_query = App\Models\Parcel::where('merchant_id', $merchant_id)
        ->where('status', '>=', 25)
        ->whereIn('delivery_type', [1, 2, 4])
        ->whereIn('payment_type', [2, 4, 6])
        ->get();
    ...
    foreach ($return_charge_amount_query as $parcel) {
        ...
        } elseif ($parcel->suborder && $parcel->exchange == 'yes' && $parcel->parent_delivery_type == 1) {
            $return_charge1 += $parcel->merchant_service_area_return_charge;
        } elseif ($parcel->suborder && $parcel->exchange == 'yes' && $parcel->parent_delivery_type == 2) {
            $return_charge1 += $parcel->merchant_service_area_return_charge;
        } elseif ($parcel->suborder && $parcel->exchange == 'no' && $parcel->parent_delivery_type == 2) {
            $return_charge1 += $parcel->merchant_service_area_return_charge;
        }
    }
    ...
    $total_pending_payment = number_format($total_customer_collect_amount - $total_charge_amount, 0, '.', '');
@endphp
