    ...
    WithProperties
{
        ...
        if (count($payments) > 0) {
            foreach ($payments as $key => $payment) {
                ...
                $data_parcel_array[] = (object) [
                    ...
                    'store_name' => $payment->merchant->parentMerchant?->company_name,
                    'store_id' => $payment->merchant->parentMerchant?->m_id,
                    'amount_to_be_collect' => number_format($total_collect_amount, 2),
                    'adjustment' => $payment->adjustment,
                    'parent_commission_amount' => $payment->parcel_merchant_delivery_payment_details->sum('parent_commission_amount'),
                    ...
                    'payable_amount' => number_format($payment->total_payment_amount, 0),
    ...
    public function map($row): array
    {
        ...
        return [
            ...
            $row->parent_commission_amount ? $row->parent_commission_amount : 0,
            $row->collected,
            $row->delivery_charge,
            $row->weight_charge,
            $row->cod_charge,
